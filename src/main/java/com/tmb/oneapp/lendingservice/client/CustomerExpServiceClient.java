...
public interface CustomerExpServiceClient {
    ...
    @PostMapping(value = "${customers-exp-service.account-loan.endpoint}", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    TmbOneServiceResponse<LoanSummary> getAccountLoan(@RequestHeader(value = LendingServiceConstant.HEADER_CORRELATION_ID, required = true) String correlationId, @RequestHeader(value = LendingServiceConstant.HEADER_X_CRMID, required = true) final String crmID, @RequestBody LoanAccountSummaryRequest loanAccountSummaryRequest);
