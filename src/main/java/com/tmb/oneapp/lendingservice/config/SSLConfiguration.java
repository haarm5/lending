...
import static com.tmb.oneapp.lendingservice.constant.LendingServiceConstant.EXCEPTION_OCCURED;
import static com.tmb.oneapp.lendingservice.constant.LendingServiceConstant.INITIALIZE_SSL_CONTEXT;
...
public class SSLConfiguration {
    ...
    private SSLContext initializeSSLContext(final String keyStorePath, final String pwKeyStore,
        ...
        logger.info(" In ", INITIALIZE_SSL_CONTEXT);
        ...
        try (final InputStream fis = this.getClass().getResourceAsStream(keyStorePath)) {
            ...
            logger.info(INITIALIZE_SSL_CONTEXT, " KMF keystorepw loaded.");
            ...
            logger.info(INITIALIZE_SSL_CONTEXT, " KMF init done.");
            ts = KeyStore.getInstance("JKS");
        } catch (UnrecoverableKeyException | KeyStoreException | NoSuchAlgorithmException | CertificateException
                | IOException exp) {
            logger.error(EXCEPTION_OCCURED, exp.getMessage());
        }
        sslContext = null;
        try (final InputStream tfis = this.getClass().getResourceAsStream(trustStorePath)) {
            if (ts != null) {
                ...
                logger.info(INITIALIZE_SSL_CONTEXT, " Truststore initialized");
            ...
            logger.error(EXCEPTION_OCCURED, exp);
        }
        if ((sslContext == null)) {
            logger.info(INITIALIZE_SSL_CONTEXT, " sslContext is null");
            System.exit(-1);
        }
        return sslContext;
    }
