...
public class LoanController {
	...
	private ResponseEntity<TmbOneServiceResponse<Object>> handleRequest(ServiceResponse serviceResponse)
		...
		throw new TMBCommonException("0001", serviceResponse.getError().getErrorMessage(),
				ResponseCode.BAD_REQUEST.getService(), HttpStatus.BAD_REQUEST, null);
	...
	@ApiImplicitParams({
			...
			@ApiImplicitParam(name = HEADER_X_CRMID, defaultValue = "001100000000000000000012004011", required = true, dataType = "string", paramType = "header") })
	public ResponseEntity<TmbOneServiceResponse<Object>> fetchProducts(
			@ApiParam(hidden = true) @RequestHeader Map<String, String> reqHeaders) throws TMBCommonException {
	...
	@ApiImplicitParams({
		@ApiImplicitParam(name = HEADER_CORRELATION_ID, defaultValue = "32fbd3b2-3f97-4a89-ae39-b4f628fbc8da", required = true, paramType = "header"),
		@ApiImplicitParam(name = HEADER_X_CRMID, defaultValue = "001100000000000000000012004011", required = true, dataType = "string", paramType = "header") })
	public ResponseEntity<TmbOneServiceResponse<ProductDetailResponse>> fetchProductOrientation(
			@ApiParam(hidden = true) @RequestHeader Map<String, String> reqHeaders, @Valid @RequestBody ProductDetailRequest request)
			throws TMBCommonException {
		String crmId = getCrmId(reqHeaders);
